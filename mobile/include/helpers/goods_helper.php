...
function get_categories_tree($cat_id = 0)
{
    ...
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $global->ecs->table('category') . " WHERE cat_id = '$cat_id'";
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $global->ecs->table('category') . " WHERE parent_id = '$parent_id' AND is_show = 1 ";
    if ($global->db->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
...
function get_child_tree($tree_id = 0)
{
    $global = getInstance();
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $global->ecs->table('category') . " WHERE parent_id = '$tree_id' AND is_show = 1 ";
    if ($global->db->getOne($sql) || $tree_id == 0)
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show ' .
                'FROM ' . $global->ecs->table('category') .
                "WHERE parent_id = '$tree_id' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC";
        $res = $global->db->getAll($child_sql);
        foreach ($res AS $row)
        {
            if ($row['is_show'])
...
function get_top10($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb, SUM(og.goods_number) as goods_number ' .
           'FROM ' . $global->ecs->table('goods') . ' AS g, ' .
                $global->ecs->table('order_info') . ' AS o, ' .
                $global->ecs->table('order_goods') . ' AS og ' .
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
               ' FROM ' . $global->ecs->table('goods') . ' AS g ' .
               ' LEFT JOIN ' . $global->ecs->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $res = $global->db->selectLimit($sql, $num);
...
function get_goods_info($goods_id)
{
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
            ...
            'FROM ' . $global->ecs->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $global->ecs->table('category') . ' AS c ON g.cat_id = c.cat_id ' .
            'LEFT JOIN ' . $global->ecs->table('brand') . ' AS b ON g.brand_id = b.brand_id ' .
            'LEFT JOIN ' . $global->ecs->table('comment') . ' AS r '.
                'ON r.id_value = g.goods_id AND comment_type = 0 AND r.parent_id = 0 AND r.status = 1 ' .
            'LEFT JOIN ' . $global->ecs->table('bonus_type') . ' AS m ' .
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT attr_group ".
            "FROM " . $global->ecs->table('goods_type') . " AS gt, " . $global->ecs->table('goods') . " AS g ".
            "WHERE g.goods_id='$goods_id' AND gt.cat_id=g.goods_type";
    $grp = $global->db->getOne($sql);
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
            ...
            'FROM ' . $global->ecs->table('goods_attr') . ' AS g ' .
            'LEFT JOIN ' . $global->ecs->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                    ...
                    'FROM ' . $global->ecs->table('goods') . ' AS g ' .
                    'LEFT JOIN ' . $global->ecs->table('goods_attr') . ' as a ON g.goods_id = a.goods_id ' .
                    "LEFT JOIN " . $global->ecs->table('member_price') . " AS mp ".
                        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
...
function get_goods_gallery($goods_id)
{
    ...
    $sql = 'SELECT img_id, img_url, thumb_url, img_desc' .
        ' FROM ' . $global->ecs->table('goods_gallery') .
        " WHERE goods_id = '$goods_id' LIMIT " . C('goods_gallery_number');
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    if ($from == 'web')
    {
        $global->tpl->assign('cat_goods_' . $cat_id, $goods);
    }
    elseif ($from == 'wap')
    {
        $cat['goods'] = $goods;
    ...
    /* 分类信息 */
    $sql = 'SELECT cat_name FROM ' . $global->ecs->table('category') . " WHERE cat_id = '$cat_id'";
...
function assign_brand_goods($brand_id, $num = 0, $cat_id = 0,$order_rule = '')
{
    ...
    /* 分类信息 */
    $sql = 'SELECT brand_name FROM ' . $global->ecs->table('brand') . " WHERE brand_id = '$brand_id'";

    $brand['id']   = $brand_id;
    $brand['name'] = $global->db->getOne($sql);
...
function get_extension_goods($cats)
{
    ...
    $sql = 'SELECT goods_id FROM ' . $global->ecs->table('goods_cat') . " AS g WHERE $cats";
    $extension_goods_array = $global->db->getCol($sql);
...
function spec_price($spec)
{
    ...
    if (!empty($spec))
    {
        ...
        $sql = 'SELECT SUM(attr_price) AS attr_price FROM ' . $global->ecs->table('goods_attr') . " WHERE $where";
        $price = floatval($global->db->getOne($sql));
    }
    else
    {
        $price = 0;
...
function group_buy_info($group_buy_id, $current_num = 0)
{
    ...
    $sql = "SELECT *, act_id AS group_buy_id, act_desc AS group_buy_desc, start_time AS start_date, end_time AS end_date " .
            "FROM " . $global->ecs->table('goods_activity') .
            "WHERE act_id = '$group_buy_id' " .
            "AND act_type = '" . GAT_GROUP_BUY . "'";
    $group_buy = $global->db->getRow($sql);
...
{
    $global = getInstance();
    $sql = "SELECT * FROM " . $global->ecs->table('goods_activity') . " WHERE act_id = '$act_id'";
    $auction = $global->db->getRow($sql);
    ...
    /* 查询出价用户数和最后出价 */
    $sql = "SELECT COUNT(DISTINCT bid_user) FROM " . $global->ecs->table('auction_log') .
            " WHERE act_id = '$act_id'";
    $auction['bid_user_count'] = $global->db->getOne($sql);
    if ($auction['bid_user_count'] > 0)
    {
        $sql = "SELECT a.*, u.user_name " .
                "FROM " . $global->ecs->table('auction_log') . " AS a, " .
    ...
    if ($auction['status_no'] > 1)
    {
        ...
        $auction['order_count'] = $global->db->getOne($sql);
    }
    else
    {
...
{
    $global = getInstance();
    $sql = "SELECT * FROM " . $global->ecs->table('favourable_activity') .
            " WHERE act_id = '$act_id'";
    $row = $global->db->getRow($sql);
    if (!empty($row))
    {
...
{
    $global = getInstance();
    $sql = "SELECT * FROM " . $global->ecs->table('wholesale') .
            " WHERE act_id = '$act_id'";
    $row = $global->db->getRow($sql);
    if (!empty($row))
    {
...
function get_goods_attr($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name " .
            "FROM " . $global->ecs->table('goods') . " AS g, " . $global->ecs->table('attribute') . " AS a " .
            "WHERE g.goods_id = '$goods_id' " .
            "AND g.goods_type = a.cat_id " .
            "AND a.attr_type = 1";
    $attr_id_list = $global->db->getCol($sql);
    $res = $global->db->query($sql);
    while ($attr = $global->db->fetchRow($res))
    {
        if (defined('ECS_ADMIN'))
        {
            $attr['goods_attr_list'] = array(0 => L('select_please'));
        }
        else
        {
    ...
    $sql = "SELECT attr_id, goods_attr_id, attr_value " .
            "FROM " . $global->ecs->table('goods_attr') .
            " WHERE goods_id = '$goods_id' " .
            "AND attr_id " . db_create_in($attr_id_list);
    $res = $global->db->query($sql);
    while ($goods_attr = $global->db->fetchRow($res))
    {
...
function get_goods_fittings($goods_list = array())
{
    ...
    $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price ".
            'FROM ' . $global->ecs->table('group_goods') . ' AS gg ' .
            'LEFT JOIN ' . $global->ecs->table('goods') . 'AS g ON g.goods_id = gg.goods_id ' .
            "LEFT JOIN " . $global->ecs->table('member_price') . " AS mp ".
                    "ON mp.goods_id = gg.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            "LEFT JOIN " . $global->ecs->table('goods') . " AS ggg ON ggg.goods_id = gg.parent_id ".
    ...
    while ($row = $global->db->fetchRow($res))
    {
        ...
        $arr[$temp_index]['parent_short_name'] = C('goods_name_length') > 0 ?
            sub_str($row['parent_name'], C('goods_name_length')) : $row['parent_name'];//配件的基本件显示的名称
        ...
        $arr[$temp_index]['short_name']        = C('goods_name_length') > 0 ?
            sub_str($row['goods_name'], C('goods_name_length')) : $row['goods_name'];//配件显示的名称
...
function get_products_info($goods_id, $spec_goods_attr_id)
{
    ...
    if(isset($goods_attr_array['sort']))
    {
        ...
        $sql = "SELECT * FROM " .$global->ecs->table('products'). " WHERE goods_id = '$goods_id' AND goods_attr = '$goods_attr' LIMIT 0, 1";
        $return_array = $global->db->getRow($sql);
    }
    return $return_array;
}
